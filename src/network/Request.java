...
public class Request extends Thread {
    ...
    private void handlePacket(byte opcode) throws IOException{
        switch(opcode){
            ...
            case OPCode.REQ_REGISTER:
                ...
                break;
            case OPCode.UPDATE_SENSOR_DATA:
                handleSensorData();
                break;
            case OPCode.UPDATE_DISTANCE:
            	handleMileageData();
            	break;
    ...
    private void handleMileageData() throws IOException {//주행거리(전체o, 부품별x) DB 업데이트 
    	String id;
    	int mileage;
    	id = dis.readUTF();
    	mileage = dis.readInt();
    	if(DBManager.getInstance().updateMileage(id, mileage)){ // id, mileage 가져오기 성공
    		dos.writeByte(OPCode.UPDATE_DISTANCE);
    		System.out.println("Update Distance Sucess");
    	}
    	else{
    		dos.writeByte(OPCode.ERR);
    		System.out.println("Error Occured! - Update Distance");
    	}
    	dos.flush();
    }
    
    private void handleDistanceData() throws IOException {
    	String id;
